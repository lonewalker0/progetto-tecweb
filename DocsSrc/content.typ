...
L'attività è stata fondamentale: abbiamo infatti potuto circoscrivere meglio i requisiti e inoltre ci ha portato a ragionare subito sulle possibili implementazioni.
...
Le principali differenze tra le due visualizzazioni si trovano nel menu di navigazione principale e nella tabella relativa agli ordini degli utenti. 
...
Inoltre è stato essenziale nel processo di validazione degli input inseriti da parte dell'utente, ogni form infatti presenta controlli lato client e produce degli errori che tramite la funzione "appendError" vengono mostrati a schermo all'interno di un determinato _div_.
...
La classe DBAccess effettua il collegamento al database e costituisce l'oggetto effettivo della connessione, mentre le query vengono effettuate tramite la classe DBoperation: per interfacciarsi al database è stata utilizzata la libreria mysqli.
=== Form e validazione degli input
Ogni form è stato configurato per essere gestito da un file PHP dedicato, utilizzando il metodo POST per la trasmissione sicura dei dati. Sono stati garantiti gli stessi controlli presenti nella validazione lato client, inoltre i messaggi di errore vengono ristampati nel medesimo contenitore utilizzato da js. 
...
La tabella _user_ in realtà non è totalmente ottimizzata, contiene infatti una campo booleano che definisce la tipologia di utente, si è comunque deciso di mantenere una tabella unica e non dividerla appunto per mantenere delle query più semplici.
Nelle tabelle Shop e Programma, per gestire le immagini, si è salvato il path della locazione delle foto.
Tutte le foto hanno una dimensione inferiore al MB, e sono stati sviluppati i relativi controlli, lato php e js, per evitare il caricamento di immagini più pesanti o di formati non accettati.
...
Per lavorare in maniera efficiente al progetto, si è deciso di adottare Docker: è stato realizzato un file docker compose che una volta avviato mettesse a disposizione del team tutte le componenti necessarie allo sviluppo nelle medesime versioni. Inoltre durante lo sviluppo si sono fatte continue prove nel server dedicato. 
